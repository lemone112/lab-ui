...
// Fast-rise model: ease-in (power < 1) for 0-8, then linear falloff to dark
function fastRiseModel(C_peak, p_rise) {
  const steps = [];
  for (let i = 0; i <= 12; i++) {
...
for (let C_peak = 0.008; C_peak <= 0.02; C_peak += 0.001) {
  for (let p_rise = 0.15; p_rise <= 0.6; p_rise += 0.02) {
    const s = score(fastRiseModel(C_peak, p_rise));
    if (s < best.score) best = { C_peak, p_rise, score: s };
  }
}

console.log(`Best fast-rise: C_peak=${best.C_peak.toFixed(4)} p_rise=${best.p_rise.toFixed(2)} total dE=${best.score.toFixed(4)}`);

const steps = fastRiseModel(best.C_peak, best.p_rise);
...
<h3>Neutral Scale Comparison</h3>
...
<div style="display:flex; gap:4px;">
  <span style="width:60px; font-size:12px; color:#666;">FastRise</span>
  ${steps.map(c => `<div style="width:48px;height:48px;border-radius:50%;background:${c};"></div>`).join('')}
</div>
</body></html>`;

await import('fs').then(fs => fs.promises.writeFile('preview-fast-rise.html', html));
console.log('\nSaved preview-fast-rise.html');
