import { differenceEuclidean, parse as culoriParse } from 'culori';
...
function toJz(hex) {
  ...
  return c.to('jzazbz').coords;
}

function toHex([jz, az, bz]) {
  const c = new Color('jzazbz', [jz, az, bz]);
  return c.to('srgb').toString({format: 'hex'});
}

const a0 = toJz('#FFFFFF');
const a6 = toJz('#787880');
const a12 = toJz('#101012');

// Interpolate in JzAzBz: Jz via ease, Az/Bz via ease-in (light) / linear (dark)
function jzModel(p_L, p_C) {
  ...
  for (let i = 0; i <= 12; i++) {
    ...
    if (i < 6) {
      ...
      jz = a0[0] + (a6[0] - a0[0]) * eL;
      az = a0[1] + (a6[1] - a0[1]) * eC;
      bz = a0[2] + (a6[2] - a0[2]) * eC;
    } else if (i === 6) {
      [jz, az, bz] = a6;
    } else {
      const t = (i - 6) / 6;
      const eL = 1 - Math.pow(1 - t, p_L);
      jz = a6[0] + (a12[0] - a6[0]) * eL;
      az = a6[1] + (a12[1] - a6[1]) * t;
      bz = a6[2] + (a12[2] - a6[2]) * t;
    }
    steps.push(toHex([jz, az, bz]));
  }
  return steps;
}
...
const toHsv = (await import('culori')).converter('hsv');
for (let i = 0; i <= 12; i++) {
